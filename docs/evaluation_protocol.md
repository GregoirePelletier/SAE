...
| 15 | Retrieval BM25 sur vocabulaire latent (Latent Terms, token-level) | `scripts/latent_retrieval_precision_eval.py` / `src/sae/retrieval/latent_terms.py --mails ...` | bloqué : OOM dans `build_token_training_pool`, aucun JSON produit | non comparé formellement à ce jour à `select_latents_by_similarity` (piste de suite) |
...
- **Fidélité de `src/sae/retrieval/latent_terms.py` au protocole Latent Terms** :
  le CODE est conforme (réimplémentation token-level, SAE entraîné sur corpus
  FineWeb2-fr générique hors-domaine, §3.1-3.2 du papier), mais aucun résultat
  n'a jamais été produit avec cette version — `build_token_training_pool` meurt
  OOM avant de terminer. Ancienne version phrase-level/en-domaine : §26/§68/§69,
  supersédés, pas citables comme résultat Latent Terms de référence.
