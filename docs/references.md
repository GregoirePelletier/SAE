...
| interp_embed | dépôt officiel (Jiang, Sun et al.) | submodule `external/interp_embed` initialisé et peuplé (28 fichiers `.py`, historique git propre) ; comparaison faite par lecture du papier uniquement, jamais par exécution | **Non défendable tel quel — c'est l'écart le plus net des cinq.** La règle de ce projet (`CLAUDE.md`, "ne pas réimplémenter une fonctionnalité déjà présente... sans comparaison documentée") est respectée pour la partie "documentée" (comparaison détaillée ci-dessus) mais pas pour la partie "sans réimplémenter sans avoir vérifié l'existant" : le code est disponible localement mais la comparaison n'a jamais été validée par une exécution réelle du code de référence sur son propre cas jouet. Correctif peu coûteux déjà identifié (F.2 de l'audit) et rendu encore moins cher par la présence du code : cloner n'est plus nécessaire, il suffit de faire tourner leur exemple depuis `external/interp_embed/` — pas encore fait. |
