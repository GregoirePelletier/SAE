...
| **Interpretable Embeddings with Sparse Autoencoders** ([nickjiang2378/interp_embed](https://github.com/nickjiang2378/interp_embed)) | Guide de conception pour le retrieval/clustering, les corrélations et la labellisation (papier : *Interpretable Embeddings with Sparse Autoencoders: A Data Analysis Toolkit*, Jiang/Sun et al. 2025). Submodule `external/interp_embed` installé et peuplé. `tests/test_interp_embed_diff.py` compare optionnellement `corpus_diff_stats` à `diff_features` d'interp_embed si le package est présent. | Choix faits en s'écartant du papier (`RESULTS_TESTS.md` §15) : similarité d'embedding plutôt que matching par sous-chaîne pour le retrieval/clustering, filtre de corrélations "intéressantes" ajouté (absent de notre implémentation initiale), gate odd-one-out retenue plutôt que génération contrastive directe (alternative documentée, non intégrée en production). Score de silhouette calculé et publié (`results.json`, P1 et P2) malgré la note 4 du papier, qui le rejette explicitement comme mesure géométrique non pertinente pour l'analyse exploratoire — gardé par continuité avec les comparaisons déjà publiées dans `RESULTS_TESTS.md`, à lire comme un indicateur de forme du nuage de points, pas une mesure de qualité d'interprétabilité. Écart non résolu : la comparaison au papier s'est faite par lecture, jamais en exécutant leur code de référence sur son propre cas jouet — le submodule est disponible localement pour ça, pas encore fait. Métriques de retrieval (App. G, `src/analysis/metrics.py`) : AP/P@K implémentées telles que données par l'équation du texte ; MAP/MP@K (moyenne d'AP/P@K sur les requêtes) et RRF (`k=60`, constante conventionnelle de Cormack et al. 2009, non précisée dans le texte de l'App. G) sont des agrégations standard non explicitées dans les Appendices — `external/interp_embed` ne contient aucun code de retrieval (vérifié sur tous les commits/branches du submodule) pour trancher. RBO implémentée en version de base (non extrapolée) : ne vaut jamais exactement 1 pour deux classements identiques de longueur finie (converge vers 1 seulement à profondeur infinie), mais reste strictement croissante avec l'accord réel entre classements — suffisant pour comparer des méthodes entre elles, cf. docstring `rank_biased_overlap`. |
| **SAE Boost** ([*Teach Old SAEs New Domain Tricks with Boosting*](https://arxiv.org/abs/2507.12990), Koriagin et al., COLM 2025) | Papier connu et utilisé comme référence de conception pendant le développement de `FrozenCoreResidualSAE`/`SAEBoostResidualSAE` (`src/sae/frozen_core.py`) : même architecture (SAE secondaire entraîné sur le résidu de reconstruction `e = x - x̂` d'un SAE core gelé, sommé à l'inférence) et même taille de dictionnaire résiduel par défaut (`D_EXTRA=1024`) que leur configuration. | Alternatives évaluées par le papier (Extended SAE random/most-active init, SAE Stitching, full fine-tuning) non testées ici faute de temps/ressources — le choix retenu s'appuie sur leur papier sans comparaison chiffrée directe à ces autres options. Deux ablations partielles depuis : `K_EXTRA=5` (§25, direction cohérente avec leur k=5 optimal, non significatif seul, n=150) ; volume testé jusqu'à 25M tokens (§23.4, même conclusion qualitative) mais le run à 100-200M (leur borne exacte, job 41658) reste à faire — cf. `docs/archive/evaluation_protocol.md`. |
| **Sanity Checks for Sparse Autoencoders** ([Korznikov et al., 2026](https://arxiv.org/abs/2602.14111)) | Non mentionné dans le cadrage initial, adopté en cours de route : leur baseline "Frozen Decoder" (décodeur figé à une initialisation aléatoire, jamais entraîné) égale un SAE réellement entraîné sur interprétabilité (AutoInterp), sparse probing et causal editing (RAVEL) dans leur étude — remettant en cause la validité de ces métriques comme preuve d'un apprentissage de features significatif. | Sanity check reproduit sur notre extension P1 (`FrozenDecoderExtendedSAE`, `src/sae/frozen_core.py`, `SANITY_CHECK_FROZEN_DECODER=1`) — cf. `RESULTS_TESTS.md` §19 pour le protocole et les résultats. |
...
- **BatchTopK + AuxK** : architecture d'entraînement SAE utilisée pour `PhraseLevelSAE`
  et l'extension `SAEBoostResidualSAE` (`src/sae/batch.py`, `src/sae/frozen_core.py`).
