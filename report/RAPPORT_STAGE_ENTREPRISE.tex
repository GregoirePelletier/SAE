...
\title{Interprétabilité automatique de représentations textuelles denses par Sparse Autoencoders\\
...
\vspace{0.5cm}
{\LARGE Interprétabilité automatique de représentations textuelles denses\\ par Sparse Autoencoders\par}
\vspace{0.3cm}
...
cross-entropy confirme que cette information est fonctionnellement exploitée par le
modèle, pas seulement présente géométriquement. Deuxièmement, le score d'interprétabilité est
...
mesure. Le protocole final retenu combine la sélection stratifiée et juge indépendant du modèle étudié. Troisièmement, l'effet d'échelle du modèle, initialement considéré
comme central, ne se réplique pas sous ce protocole : sur Gemma 3 1B, 4B, 12B et 27B (version "instruct"), aucune
...
d'embeddings documentaires) plutôt que de réimplémenter, et de justifier
...
Avant de détailler les variantes, un schéma de principe : un SAE prend un
vecteur d'activation \emph{dense} (peu de dimensions, toutes non nulles,
chacune mélangeant plusieurs concepts) et le ré-encode dans un espace de
beaucoup plus grande dimension mais \emph{parcimonieux} (l'immense majorité
des coordonnées sont nulles pour un texte donné) avant de le décoder pour
reconstruire l'entrée d'origine. L'idée est que chaque coordonnée non nulle
de ce code, plus nombreuses mais individuellement plus rares, a de meilleures
chances de correspondre à un concept isolé et nommable.

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\textwidth]{sae_arch.png}
\caption{Principe d'un Sparse Autoencoder : (a) des activations sont
échantillonnées à un point donné d'un modèle de langage ; (b) un dictionnaire
de features est appris en représentant chaque activation comme une
combinaison linéaire parcimonieuse de directions apprises (encodeur/décodeur,
poids liés) ; (c) chaque feature du dictionnaire obtenu est interprétée
individuellement.}
\label{fig:sae-principle}
\end{figure}
...
Ce stage a développé \textbf{deux pipelines indépendants} (détaillés au
chapitre~\ref{sec:contribution}) : un pipeline principal qui étend un SAE
préentraîné sur les activations internes de Gemma-3 (GemmaScope), et un
second pipeline qui entraîne un SAE directement sur des embeddings de
phrase, sans dépendre de GemmaScope. Ce second pipeline s'appuie
...
Un SAE dispose d'un budget fixe de features actives par exemple ($k$ dans
BatchTopK) : c'est comme un formulaire à un nombre limité de cases à cocher
pour décrire chaque texte. Si l'on réentraînait un unique SAE sur un mélange
de texte générique et de texte du domaine cible (les mails), les concepts
des deux origines devraient se partager les \emph{mêmes} cases --- au risque
qu'une case en vienne à désigner plusieurs concepts sans rapport (\emph{ex.}
\og{}météo\fg{} et \og{}facturation\fg{} sur la même case), ce qui rend la
feature inutilisable pour l'interprétation. \textbf{Geler le cœur
préentraîné et n'entraîner qu'une extension sur son résidu de reconstruction}
évite ce partage par construction : le cœur garde ses cases telles
qu'apprises sur son corpus d'origine, et l'extension dispose de cases
\emph{supplémentaires}, dédiées uniquement à ce que le cœur ne capture pas
déjà --- sans jamais entrer en concurrence avec lui pour la même case.
\end{definitionbox}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{SAE_micro.png}
\caption{Passage du niveau token au niveau document : les activations SAE de
chaque token du document sont agrégées par maximum, coordonnée par
coordonnée, en un unique vecteur documentaire $v$ --- une feature est
\og{}active pour le document\fg{} si elle s'active fortement sur au moins un
de ses tokens.}
\label{fig:sae-micro}
\end{figure}

Le protocole \emph{odd-one-out}
...
Un dashboard interactif (Streamlit) donne accès aux artefacts produits par
ces deux pipelines sans recalcul : vue d'ensemble des métriques par run,
projection UMAP interactive des documents, exploration des features (labels
Neuronpedia et juge LLM, exemples positifs/négatifs), résultats de diffing,
recherche par mot-clé sur les labels, et diagnostics d'entraînement ---
capture d'écran \S\ref{sec:contribution}, page \pageref{fig:dashboard} (page
\og{}Features\fg{}, feature 17342, en regard de l'exemple commenté juste
après).
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{saeboost_architecture.png}
\caption{Schéma de l'architecture à cœur gelé, reproduit de Koriagin et
al.~\cite{koriagin2025saeboost} (Figure 1) --- dans la terminologie de ce
rapport, \og{}Domain-Specific SAE\fg{} correspond à l'\textbf{extension}
(entraînée sur le résidu $e$), \og{}General SAE\fg{} au \textbf{cœur
GemmaScope gelé}, et \og{}Stitched SAE\fg{} à leur somme $\hat{x}$ utilisée
à l'inférence. \textbf{Gauche} : à l'entraînement, seule l'extension apprend,
sur le résidu du cœur gelé. \textbf{Droite} : à l'inférence, les deux
décodeurs sont sommés.}
\label{fig:saeboost-arch}
\end{figure}
...
\begin{resourcebox}[Exemple réel --- feature 17342 jugée sous le protocole final (R0)]
\label{box:feature17342}
Le juge reçoit 9 extraits dont l'activation de la feature 17342 est forte, et
1 extrait-contrôle dont l'activation est quasi nulle (adresses et coordonnées
anonymisées ci-dessous, comme pour tout le corpus --- \emph{cf.}~encadré
\S\ref{sec:etat-art}). Le mot entre chevrons \texttt{<<~~>>} est, pour chaque
extrait, le token précis où l'activation de la feature est la plus forte
(c'est autour de lui que le contexte affiché au juge est centré) :
\begin{quote}\small
\textbf{1.} \og{}Je me permets de vous solliciter afin de vous faire part
d'une situation préoccupante que je rencontre actuellement à mon domicile.
Hier, aux environs de \texttt{<<17h00>>}\ldots{}\fg{}

\textbf{2.} \og{}[\ldots] dans mon domicile situé [adresse]. J'ai en effet
été \texttt{<<victime>>}\ldots{}\fg{}

\textbf{3.} \og{}C vraiment urgent ! J'ai eu une grosse
\texttt{<<coupure>>}\ldots{}\fg{}

\textbf{4 (contrôle).} \og{}Je vous écris concernant le raccordement
électrique de mon nouveau logement, situé au 17 Avenue de Mouzieys,
\texttt{<<[code postal]>>}\ldots{}\fg{}
\end{quote}
Malgré des registres très différents (formel, familier), les extraits 1 à 3
partagent un thème identifiable --- le signalement d'un incident survenu au
domicile --- que l'extrait 4 (une demande de raccordement, pas un incident)
ne partage pas. Un juge qui désigne correctement l'extrait 4 comme l'intrus
démontre l'existence de ce concept partagé ; le libellé retenu pour cette
feature, \textbf{\og{}Signalement incident\fg{}}, en est la trace.
\textbf{Point de vigilance} : le token marqué sur l'extrait-contrôle est un
code postal, pas un mot de nature différente des trois autres --- ce n'est
\emph{pas} une coïncidence sémantique, c'est la conséquence directe du
correctif décrit \S\ref{subsec:sanity-check-resultats} (le contrôle est
choisi parmi des documents où la feature ne s'active quasiment jamais ; sa
position affichée est alors tirée aléatoirement, sans rapport avec son
contenu, précisément pour éviter qu'un juge ne la reconnaisse par une
position ou un motif de surface plutôt que par l'absence du concept). Les
trois positifs, à l'inverse, sont bien centrés sur des mots directement liés
au signalement d'un incident (\og{}victime\fg{}, \og{}coupure\fg{}), pas sur
un artefact de position. \emph{6 autres extraits positifs, omis ici pour la
lisibilité, ont été présentés au juge dans le run réel.}
\end{resourcebox}

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\textwidth]{Streamlit_feature17342.png}
\caption{Page \og{}Features\fg{} du dashboard interactif, run de référence R0
(\texttt{results\_v27\_ablation\_classic\_setup\_k5\_25m\_layer31}) : les
mêmes exemples positifs et le même contrôle négatif que l'encadré
ci-dessus, avec le token d'activation maximale entre chevrons pour chaque
extrait --- l'outil réel produisant l'exemple que le texte vient de
discuter, pas une reconstruction.}
\label{fig:dashboard}
\end{figure}
...
Le split train/test est effectué au niveau du \textbf{mail d'origine} pour éviter qu'une variante augmentée d'un mail de test ne fuite
...
Le taux d'interprétabilité étant une proportion mesurée sur un échantillon restreints du nombre de features totales, chaque comparaison entre deux configurations est testée
...
\subsection{Première pipeline de validation}
\label{subsec:diagnostic-initial}

Un premier corpus (corpus generique énergie/sport/support), jugé sur un échantillon initial de dix de
features seulement échouait le test \emph{odd-one-out} dans la grande majorité des cas (8/10).
...
ensemble). A ce stade, le réservoir de résidus servant à entraîner l'extension était
...
d'entraînement de l'extension, seulement dans une visualisation post-hoc, d'où ces premiers résultats négatifs.

Une fois le corpus principal reconstruit (mails + variantes augmentées,
\S\ref{sec:contribution}), le taux d'interprétabilité mesuré sous protocole
R0 (sélection stratifiée, juge Qwen3.8-27B, négatif corrigé, déduplication
par mail parent) atteint \textbf{65,7\% (197/300)} sur le corpus emails
(\emph{cf.}~\S\ref{subsec:correction-methode}). Faire varier le volume de
tokens d'entraînement de l'extension d'un facteur 20 (100k à 2M tokens), à
domaine fixé, ne produit \textbf{aucun effet mesurable} (résultat obtenu sous
sélection par magnitude, non repris depuis sous R0 --- \emph{cf.}~
Table~\ref{tab:synthese} en annexe, rangs \og{}Volume\fg{}). Le contraste
initial (8/10 échecs sur un échantillon non exploitable de 10 features)
reposait sur un déficit de puissance statistique, pas sur une mesure fiable.

\begin{resultbox}[L'effet domaine ne se distingue pas sous R0]
Corpus générique (energy/sports/support) et corpus emails, comparés à
effectif apparié ($n=300$ chacun) sous protocole R0 intégral ($K_\text{extra}=5$,
$D_\text{extra}=1024$, 25M tokens, layer 31 --- extraction et entraînement
complets pour le corpus générique, aucun checkpoint sous ces hyperparamètres
n'existant au préalable) : \textbf{67,7\% (203/300)} pour le corpus générique
contre \textbf{65,7\% (197/300)} pour le corpus emails (R0) --- $z=0{,}52$,
$p=0{,}60$, \textbf{non significatif}, le point estimé s'inversant même très
légèrement (générique au-dessus). \textbf{Ni le domaine ni le volume du
corpus d'entraînement ne montrent d'effet détectable} sur le taux
d'interprétabilité une fois la sélection stratifiée et le juge indépendant
appliqués. Cette conclusion rejoint celles déjà tirées pour l'effet d'échelle
du modèle (\S\ref{subsec:model-scale}) : un effet mesuré comme significatif
sous sélection par magnitude et juge auto-référent lors d'une campagne
antérieure de ce stage (Table~\ref{tab:synthese} en annexe) qui ne survit pas
à la correction complète du protocole. Deux domaines aussi éloignés que
possible (mails clients \emph{vs.} Wikipédia/web générique) donnant un
résultat statistiquement indiscernable est en soi l'argument le plus fort
disponible contre un effet domaine détectable à ce protocole et à ce $n$ ; la
taille d'effet minimale détectable ici reste ${\sim}10$ points
(\S\ref{subsec:ablations-finales}), un effet réel mais plus modeste
...
battre), un décodeur figé aléatoire laisse l'extension
...
(\emph{iso}) et $+0{,}0086$ (\emph{cov}) contre $+0{,}1393$ pour R0. \textbf{Même sous le
schéma le plus favorable au décodeur aléatoire, le SAE entraîné explique
${\sim}16\times$ plus de variance résiduelle supplémentaire.}Les rares features vivantes donnent
\textbf{72,7\% ($n=11$) et 74,2\% ($n=31$)} --- statistiquement indiscernables
de R0 ($p=0{,}63$ et $p=0{,}34$). C'est le $\Delta$FVE, indépendant du bruit de
jugement, qui reste le test de
falsification net de ce sanity check --- pas le taux d'interprétabilité,
dont ce résultat confirme une fois de plus la fragilité comme métrique
isolée.

Cet audit du document négatif n'est pas, en soi, un résultat scientifique
nouveau sur la représentation apprise : c'est un \textbf{contrôle de
validité du protocole}, qui conditionne la lecture de toutes les mesures
d'interprétabilité de ce rapport --- et non la contribution centrale du
stage, malgré l'ampleur de son effet numérique.

\begin{resultbox}[Biais de construction du négatif \emph{odd-one-out} --- un contrôle de validité du protocole]
...
affiché renvoie systématiquement l'indice 0 (convention numpy sur les ex-aequo). \textbf{Un juge
peut alors résoudre la tâche par simple longueur/pauvreté de contexte, sans
lire le concept partagé par les 9 autres exemples}. Le correctif retenu
...
\textbf{Rejugé sous ce correctif à $n=150$, R0 tombe de 84,7\% à 62,7\%}. Rejugé à $n=300$ pour
resserrer la précision, R0 donne \textbf{65,7\% (197/300)}, IC95\%
$[60{,}1\%\,;\,70{,}8\%]$ : c'est la valeur de référence retenue pour la suite de
ce rapport.
\end{resultbox}
...
Sous R0 ($K_\text{extra}=5$), cette mesure compare directement le décodeur entraîné aux
mêmes témoins à décodeur \textbf{aléatoire} que le sanity check ci-dessus (checkpoints
C1/iso et C1b/cov, réutilisés sans réentraînement), sur un échantillon apparié de 60
documents tenus à l'écart : le patch causal est restreint aux positions sur lesquelles
l'extension a réellement été entraînée (\texttt{extract\_residual\_acts} exclut
systématiquement le premier token et les rares outliers de norme intra-document liés aux
\emph{massive activations} de Gemma-3 de tout entraînement et de tout calcul de
$\Delta$FVE ; patcher ces positions produirait une dégradation hors-distribution non
représentative). Le décodeur entraîné dégrade nettement moins la cross-entropy que les
deux témoins aléatoires (\textbf{$\Delta$CE 0,301} contre 0,525 et 0,510) :
$p=7{,}5\times10^{-11}$ (iso) et $p=1{,}6\times10^{-10}$ (cov), test de Wilcoxon
signed-rank apparié, 56/60 et 54/60 documents en faveur du SAE entraîné. Ce résultat est
particulièrement important parce qu'il est continu et indépendant du juge --- il montre
que l'information reconstruite par l'extension est fonctionnellement exploitée par le
modèle, pas seulement présente géométriquement dans le résidu.

\begin{resultbox}[Validation architecturale]
Les deux métriques convergent, chacune contre un témoin à décodeur \textbf{aléatoire} sous
R0 : le $\Delta$FVE montre que le décodeur entraîné récupère une quantité substantielle
d'information résiduelle ($+0{,}1393$ sous R0, contre $+0{,}0001$ à $+0{,}0086$) ; le
$\Delta$CE montre que cette information intervient causalement sur la prédiction du modèle
(dégradation de cross-entropy réduite à 0,301 contre 0,51--0,53 pour les témoins aléatoires,
$p\approx10^{-10}$). Le taux d'auto-interprétation ne constitue donc pas le seul argument en
faveur de l'architecture à cœur gelé.
\end{resultbox}

\subsection{Le protocole d'évaluation comme variable expérimentale}
...
ce facteur avec le choix de la population de features. Les deux variables sont croisées sur une configuration commune
...
mais pas comme estimations finales.
\end{resultbox}
...
dédié et son propre \emph{layer} à \og{}~2/3 de profondeur\fg{} (conseillé par la littérature car contenant a priori plus d'information sémantique que les premières et dernières couches)
...
ordonnée), mais un motif compatible avec du bruit : R0 est la configuration la plus
...
entre les deux graines, un plancher de bruit run-à-run serré. Sur les 15 comparaisons de ce chapitre testées contre R0
...
contre 65,7\% pour $K_\text{extra}=5$, une chute de plus de 11 points, le défaut historique du
dépôt ($K_\text{extra}=32$) est identifié comme la seule ablation
...
part, les trois graines de R0 (65,7\%, 63,7\%, 64,0\%, \emph{cf.}~\S\ref{subsec:ablations-finales})
donnent des taux globaux très proches, mais le recouvrement exact des labels attribués
aux features individuelles entre graines reste faible : \textbf{14,4\%} entre R0 et sa
première réplication (seed 123), \textbf{11,9\%} entre les deux répliques (seeds 123 et
7) --- mesuré directement sur les trois graines de la campagne finale, sous protocole
R0 intégral. Cela suggère qu'une bonne performance globale ne garantit pas
l'identité des mêmes directions latentes d'un entraînement à l'autre ; ce recouvrement
est même plus faible que la première estimation obtenue en cours de campagne (28,2\%,
sous sélection par magnitude et juge auto-référent, non comparable à R0) --- la
sélection stratifiée, qui échantillonne aussi les régions rares du dictionnaire, plus
idiosyncrasiques d'une graine à l'autre, explique probablement l'essentiel de cet
écart plutôt qu'un changement réel de robustesse du SAE.
...
question d'interprétabilité d'une question d'utilité prédictive. Les
...
sélection de features, ni de la construction du négatif.
\end{resultbox}

\subsection{Robustesse du protocole de jugement}
\label{subsec:robustesse}

Deux tests évaluent, sous protocole R0 intégral ($n=300$, stratifié,
Qwen3.8-27B, déduplication par mail parent), la sensibilité du protocole
\emph{odd-one-out} à des perturbations de surface qui ne devraient, en
principe, pas affecter le jugement :

\begin{itemize}
  \item \textbf{Réordonnancement} : en répétant la question 5 fois par
    feature avec un ordre de présentation différent, \textbf{53,3\%} des
    features obtiennent une décision unanime. Le taux agrégé bouge peu
    (single-shot 65,7\% contre vote majoritaire 63,7\%, test de McNemar
    $p=0{,}51$, non significatif) mais \textbf{19,3\% des features changent
  ...
  \item \textbf{Langue du juge} \cite{resck2025multilingual, multilingualconcepts2025} : traduire les exemples et le prompt en
    anglais ne change pas significativement le taux agrégé (français 65,5\%
    contre anglais 63,5\%, McNemar $p=0{,}58$), mais \textbf{27,0\% des
...
production. Ce bruit individuel est net mais plus contenu que ne le
suggérait une première mesure obtenue en cours de campagne (respectivement
31,3\% et 38,6\% de bascule, sous sélection par magnitude et juge
auto-référent) --- cohérent avec le motif déjà observé pour d'autres
résultats de ce rapport (le protocole historique amplifiait les artefacts
de mesure plutôt que de simplement les répliquer à l'identique).
...
\subsection{Fidélité du steering}
...
protocole révèle, sous R0 ($K_\text{extra}=5$, layer 31), sur les 10
features les plus explicatives de chacune des 5 intentions, un comportement
\textbf{très hétérogène selon l'intention} : amplifié pour réclamation
(4,03$\times$), résiliation (1,53$\times$) et urgence (2,65$\times$),
partiellement préservé pour information (0,58$\times$), et quasi totalement
neutralisé pour remboursement (0,02$\times$). Le steering n'est donc
\textbf{pas} un mécanisme d'intervention causale fiable et prévisible en
...
\textbf{Note de stabilité} : la répartition précise par intention n'est pas
stable d'une configuration à l'autre du SAE --- mesurée sur le checkpoint
historique ($K_\text{extra}=32$, layer 24), la même chaîne donnait urgence
fortement neutralisée (0,11--0,13$\times$) plutôt qu'amplifiée, information
quasi totalement préservée (0,97--0,99$\times$) plutôt que partiellement, et
remboursement partiellement préservé (0,28--0,37$\times$) plutôt que quasi
neutralisé. Seule la conclusion générale (effet hétérogène selon l'intention,
absence de mécanisme causal uniforme) est robuste aux deux configurations ;
les catégories précises par intention ne le sont pas et ne doivent pas être
lues comme une propriété stable d'une intention donnée.
...
par des \textbf{labels de features SAE} : chaque document est représenté par
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\textwidth]{latent_terms_mixedbread.png}
\caption{Principe de Latent Terms (illustration Mixedbread AI, éditeurs de
Clavié et al.~\cite{clavie2026latentterms}) : les embeddings de tokens d'un
passage sont projetés par le SAE en features latentes nommées (\og{}F365 :
Soccer player roles\fg{}, \emph{etc.}), indexées et classées par BM25 comme
des mots-clés classiques --- ici sur un exemple générique (football), le
principe étant identique sur le vocabulaire latent appris sur les mails.}
\label{fig:latent-terms}
\end{figure}
...
Le \emph{diffing} consiste à repérer les features dont l'activation diffère entre
deux sous-corpus, pour faire émerger ce qui les distingue.
...
de features à juger (\S\ref{subsec:correction-methode}) --- porter cette
même chaîne sur un diffing emails reste la suite naturelle de ce résultat.

\subsection{Clustering interprétable de documents}

\label{subsec:clustering}

Le clustering interprétable de documents a été exercé de bout en bout sur le
corpus emails, sous R0, sur le jeu de 197 features interprétables de R0
(\emph{cf.}~\S\ref{subsec:correction-methode}). À partir d'une requête en
langage naturel (\emph{ex.} \og{}type de réclamation client\fg{}), un juge
LLM génère des mots-clés, sélectionne les latents de l'extension dont les
labels s'y rapportent (union des \emph{top-k} par mot-clé), puis regroupe
les documents par similarité de Jaccard sur leur profil d'activation binaire
sur ces latents. Chaque cluster obtenu est ensuite étiqueté par un juge LLM,
son accuracy mesurée par réassignation LLM à l'étiquette la plus proche, et
sa \textbf{compacité géométrique} comparée, en espace d'embedding dense
(bge-m3), à des échantillons aléatoires de même taille (\emph{z-conductance} :
négatif si le cluster est plus dense que le hasard) :

\begin{table}[h!]
\centering
\caption{Clustering ciblé sur la requête \og{}type de réclamation client\fg{}, sous R0 (n=300 documents, 4 clusters)}
\label{tab:clustering}
\vspace{0.4em}\small
\begin{tabular}{@{}clcc@{}}
\toprule
\textbf{Cluster} & \textbf{Description (LLM)} & \textbf{Accuracy} & \textbf{$z$-conductance} \\ \midrule
0 (41 doc.) & Réclamations agressives, coupures de courant & 29,3\% & $-1{,}39$ \\
1 (56 doc.) & Réclamations mixtes formelles, service et facturation & 64,3\% & $-5{,}09$ \\
2 (111 doc.) & Réclamations formelles standard, coupures de courant & 27,9\% & $-10{,}86$ \\
3 (92 doc.) & Résiliation de contrat / nouvelles demandes de service & 90,2\% & $-15{,}94$ \\
\bottomrule
\end{tabular}
\end{table}
...
dense, que des échantillons aléatoires de même taille --- un effet même plus
marqué que sur une première mesure exploratoire obtenue en cours de
campagne, sur un jeu de features distinct issu de la sélection par magnitude
(jusqu'à $z=-10{,}06$ au plus négatif, contre $z=-15{,}94$ ici). L'accuracy
varie en revanche fortement d'un cluster à l'autre (27,9\% à 90,2\%),
cohérent qualitativement avec la littérature citée sur des clusters SAE à
variance d'accuracy plus élevée que des baselines denses. La répartition
précise par cluster (tailles, contenu, accuracy par cluster) n'est pas
directement comparable entre les deux mesures : le jeu de features R0 n'est
pas un sur-ensemble de celui de la première mesure, la sélection stratifiée
et la sélection par magnitude n'échantillonnant pas la même population de
features.
...
tirage de corpus par condition.
...
production). Le clustering interprétable de ce dépôt n'est donc validé que
lorsqu'il est guidé par une requête en langage naturel qui restreint
l'espace de recherche --- une exploration non supervisée du dictionnaire
complet ne fait pas émerger, à elle seule, une structure sémantique
détectable sur ce corpus.
...
la dimension d'embedding dégrade les performances graduellement, pas abruptement (10\% des
...
contexte) : vérifié directement, négatifs et positifs
...
comparaison plutôt qu'un test par défaut unique (test à deux proportions
indépendantes par défaut, McNemar pour les plans appariés, Cochran-Armitage
pour l'effet dose-réponse de l'échelle du modèle) --- et la correction
Benjamini-Hochberg n'a été appliquée qu'à la campagne finale à $n=300$
(ci-dessus) ; la campagne historique (Table~\ref{tab:synthese}, en annexe),
non corrigée, doit être lue en conséquence.
...
Partie bibliographique sur les SAE & Panorama des
...
Proposition originale & Deux pipelines indépendants
...
\begin{enumerate}
  ...
  $\Delta$CE fournissent deux validations indépendantes du juge LLM, toutes deux contre
  un témoin à décodeur aléatoire sous R0 : l'extension apprend une information
  résiduelle substantielle que les décodeurs figés aléatoires ne reproduisent pas
  ($\Delta$FVE), et cette information est causalement utile au modèle plutôt que
  purement géométrique ($\Delta$CE, $p\approx10^{-10}$).
  ...
  elles peuvent déplacer fortement
  le taux "d'interprétabilité" observé.
...
évaluer une utilité indépendante du score d'explication.

\subsection{Apports pour EDF et apports personnels}

\textbf{Pour EDF R\&D}, le stage livre une architecture réutilisable et validée
indépendamment du juge LLM ($\Delta$FVE/$\Delta$CE), un protocole d'auto-interprétation
audité et documenté (sélection stratifiée, juge indépendant, négatif exempt d'artefact
de longueur) directement réutilisable pour éviter de reproduire les biais de mesure
identifiés ici, une démonstration chiffrée que les codes SAE égalent ou battent une
baseline lexicale honnête sur cinq tâches d'intention client et un dashboard interactif opérationnel avec les
scripts d'ablation associés, réutilisables pour toute extension à d'autres corpus ou
cas d'usage (priorisation, détection de réclamation).

\textbf{Pour le stagiaire}, le stage a été l'occasion de développer une méthodologie de
recherche par ablation contrôlée et audit statistique (tests appariés, correction pour
comparaisons multiples, analyse de puissance), une pratique concrète de l'infrastructure
GPU à grande échelle (orchestration SLURM, gestion mémoire, caches partagés entre runs),
et la capacité à
remettre en cause une hypothèse de travail annoncée (l'effet d'échelle du
modèle), une compétence
de rigueur scientifique transférable au-delà de ce projet spécifique.

\subsection{Points non résolus et perspectives}

\textbf{Pistes délibérément abandonnées.} Au vu de l'ampleur des expérimentations,
...
référence) n'était pas viable sur l'architecture des GPUs partagés. L'absence d'effet mesurable du
volume jusqu'à 25M (Table~\ref{tab:synthese}) reste donc non extrapolée au
-delà de cette borne.

\textbf{Modèles d'embeddings.} CamemBERTav2 (modèle d'embedding encodeur compétitif sur le français technique) et Qwen3-Embedding,
identifiés en phase bibliographique comme candidats sérieux pour le
Pipeline~2, n'ont pas été intégrés faute de temps.
...
mesure indépendante fondée sur la cross-entropy confirme que cette information est
causalement exploitée par le modèle, pas seulement présente géométriquement.
Ces résultats ne dépendent ni du juge LLM ni du document négatif du protocole
\emph{odd-one-out}. Ce choix architectural mérite d'être rappelé explicitement :
le Pipeline~1 de ce stage n'a \textbf{jamais réentraîné un SAE from-scratch} sur
les activations de Gemma-3 --- il gèle le dictionnaire GemmaScope déjà
préentraîné par Google DeepMind et n'entraîne qu'une extension légère sur son
résidu de reconstruction (\S\ref{subsec:frozencore}). C'est un choix de
sobriété computationnelle autant qu'un choix scientifique : il évite le coût
et le risque d'oubli catastrophique d'un réentraînement complet, au prix de
n'apprendre que ce que le cœur préentraîné ne capture pas déjà.
...
une variable scientifique à part entière. La phase expérimentale a conduit à retenir 65,7\% comme
...
auto-référent (à lire comme plancher relatif, pas comme
taux réel ; \emph{cf.}~\S\ref{subsec:model-scale} pour les valeurs
correspondantes sous protocole final)}
\label{tab:synthese}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
Les expériences de diffing sur corpus générique, le clustering ciblé sur une requête,
le balayage MRL et le run à 100M et 200M tokens (décris par Teach Old SAEs new domain trick comme borne inférieure de tokens permettant d'obtenir pour l'extension du SAE des directions orthogonales au direction de l'espace latent du SAE initial) constituent des pistes utiles mais ne doivent
pas être présentés comme des validations définitives. Les limitations détaillées au
chapitre Discussion précisent les conditions nécessaires à leur consolidation.
