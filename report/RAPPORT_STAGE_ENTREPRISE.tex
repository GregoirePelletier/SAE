...
jugement. Les deux datent de la sélection par magnitude et du juge
auto-référent, antérieurs au protocole final R0 --- les pourcentages absolus
ci-dessous ne sont donc pas comparables à 65,7\%, mais la propriété
qu'ils démontrent (le protocole \emph{odd-one-out} à décision unique est
bruyant à l'échelle de la feature individuelle) est structurelle, indépendante
de la version du protocole testée :
...
un comportement \textbf{très hétérogène selon l'intention} : amplifié pour
réclamation (1,87$\times$), partiellement préservé pour résiliation
(0,64--0,82$\times$) et remboursement (0,28--0,37$\times$), quasi totalement
préservé pour information (0,97--0,99$\times$), et fortement neutralisé pour
urgence (0,11--0,13$\times$). Le steering n'est donc \textbf{pas} un
mécanisme d'intervention causale fiable et prévisible en sortie réelle : une
intervention efficace dans l'espace latent peut s'avérer sans effet une fois
le résultat effectivement produit.

\textbf{Note de traçabilité} : la première mesure de ce protocole, obtenue
avant la correction d'un bug de regex sur les labels d'intention
(\S\ref{subsec:lexical-leakage}), catégorisait à tort \emph{information}
comme neutralisée et \emph{urgence} comme préservée --- l'inverse du résultat
ci-dessus, rejoué sous labels corrigés et confirmé sur deux graines
indépendantes du classifieur. La conclusion qualitative générale (effet
hétérogène, pas de mécanisme causal uniforme) est inchangée par cette
correction ; les catégories par intention, elles, ne l'étaient pas.
...
corpus emails, sur le jeu de 68 features interprétables issu de la campagne
historique (sélection par magnitude, juge auto-référent, antérieure au
protocole R0) --- un résultat exploratoire, jamais rejoué sur les features
sélectionnées et jugées sous protocole final, à lire comme une démonstration
de faisabilité de la chaîne plutôt que comme une mesure de référence. À
partir d'une requête en langage naturel (\emph{ex.}
...
des \emph{top-k} par mot-clé, 61 features sur les 68 ci-dessus),
puis regroupe les documents par similarité de Jaccard sur
leur profil d'activation binaire sur ces latents. Chaque cluster obtenu est ensuite étiqueté par un juge LLM, son
...
dense, que des échantillons aléatoires de même taille. L'accuracy varie en revanche fortement
