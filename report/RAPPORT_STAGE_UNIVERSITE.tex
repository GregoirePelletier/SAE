...
${\sim}16\times$ plus de variance résiduelle supplémentaire.} Rejugées sous
le correctif de négatif ci-dessous, les rares features vivantes donnent
\textbf{72,7\% ($n=11$) et 74,2\% ($n=31$)} --- statistiquement indiscernables
de R0 ($p=0{,}63$ et $p=0{,}34$), contrairement à l'ancien protocole où
l'écart apparaissait significatif ($z=2{,}86$) : \textbf{le signal distinguant
décodeur entraîné et aléatoire sur le seul taux d'interprétabilité ne survit
pas à la correction du négatif}, à la faible puissance disponible sur
11/31 features vivantes. C'est le $\Delta$FVE, indépendant du bruit de
jugement et de la construction du négatif, qui reste le test de
...
\begin{figure}[h!]
\centering
\IfFileExists{figures/delta_fve_bars.png}{\includegraphics[width=0.7\textwidth]{figures/delta_fve_bars.png}}{\fbox{\parbox[c][4cm][c]{0.65\textwidth}{\centering Figure unavailable in standalone review export}}}
...
ce rapport. \textbf{Ce biais ne réfute pas les résultats en $\Delta$FVE
ci-dessus} (métrique continue, indépendante de la construction du négatif),
mais il invalide la valeur absolue de tout taux d'interprétabilité cité dans
les versions antérieures de ce document, y compris l'ancienne référence
84,7\% --- \textbf{ainsi que la conclusion d'absence d'effet d'hyperparamètre
et d'effet d'échelle du modèle qui en avait été tirée} : rejugés à $n=300$
...
$K_\text{extra}$ fait exception --- et l'effet d'échelle du modèle, présenté
plus haut comme le résultat central du stage, ne réplique pas du tout sous
...
qui a ensuite conduit à définir le protocole final. Le chiffre de référence retenu
...
\begin{figure}[h!]
\centering
\IfFileExists{figures/model_scale_curve.png}{\includegraphics[width=0.75\textwidth]{figures/model_scale_curve.png}}{\fbox{\parbox[c][4cm][c]{0.7\textwidth}{\centering Figure unavailable in standalone review export}}}
...
\begin{figure}[h!]
\centering
\IfFileExists{figures/d1_intent_bars.png}{\includegraphics[width=0.85\textwidth]{figures/d1_intent_bars.png}}{\fbox{\parbox[c][4cm][c]{0.8\textwidth}{\centering Figure unavailable in standalone review export}}}
...
$\rho=0,069$, $p=0,40$) --- le biais de \S\ref{subsec:sanity-check-resultats}
...
évaluer une utilité indépendante du score d'explication. Cette organisation est plus
informative qu'une lecture chronologique des nombreuses ablations réalisées au cours
du stage.
...
baseline lexicale honnête sur cinq tâches d'intention client --- l'argument le plus
direct pour un jury industriel --- et un dashboard interactif opérationnel avec les
...
et --- ce que ce rapport a cherché à rendre visible plutôt qu'à effacer --- la capacité à
remettre en cause une hypothèse de travail annoncée publiquement (l'effet d'échelle du
modèle) lorsque l'audit du protocole qui la soutenait ne la confirme plus, une compétence
...
le balayage MRL et le run à 200M tokens constituent des pistes utiles mais ne doivent
