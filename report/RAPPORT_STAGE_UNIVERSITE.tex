...
d'interprétabilité. Seule l'\textbf{échelle du modèle extracteur/juge} produit
un effet net, monotone et hautement significatif, qui plafonne au-delà d'un
certain seuil de taille. Sous la configuration retenue comme référence pour ce
rapport, le taux d'interprétabilité atteint \textbf{94,7\%}. Des tests
complémentaires --- fidélité et plausibilité de l'explication au niveau
document, robustesse du protocole de jugement, biais multilingue, fidélité du
steering, évaluation quantitative du retrieval --- achèvent la validation du
système.
...
effect} of any of these levers on the interpretability rate. Only the
\textbf{extractor/judge model's scale} produces a clean, monotonic, highly
significant effect that plateaus beyond a certain size threshold. Under the
configuration adopted as this report's reference, the interpretability rate
reaches \textbf{94.7\%}. Complementary tests --- document-level explanation
...
\textbf{Ce résultat a lui-même été requalifié} en remesurant le point 1B sous
méthodologie pleinement corrigée (sélection stratifiée, juge Qwen,
déduplication des exemples positifs --- \S\ref{subsec:correction-methode}) :
\textbf{80,0\% à 1B contre 94,0\% à 12B}, écart toujours significatif
($z=-3{,}61$, $p=0{,}0003$) mais \textbf{deux fois plus modeste} que le
chiffre historique ($h$ de Cohen $=-0{,}43$, effet moyen, contre $-0{,}77$,
effet large ; écart brut de 14,0 points au lieu de 33,3). Une partie
substantielle de l'écart 1B/12B initialement rapporté était donc un artefact
des biais de sélection et de juge (le 1B, jugé par un modèle auto-référent
sur des features sélectionnées par magnitude, était pénalisé deux fois) ---
\textbf{l'effet d'échelle reste le résultat le plus robuste de ce stage, mais
son ampleur rapportée ici surestimait l'effet réel d'un facteur proche de
deux.}
\end{resultbox}
