...
posteriori. Les Sparse Autoencoders (SAE), qui réapprennent une
représentation parcimonieuse dans laquelle chaque direction correspond dans
l'idéal à un concept isolé et nommable en langage naturel, sont étudiés ici
comme réponse à ce besoin.
...
indépendant du premier, ne s'appuie plus sur les activations internes d'un
...
\textbf{stratifiée par fréquence d'activation} plutôt que par magnitude, pour
représenter l'ensemble du dictionnaire plutôt que ses seules directions les
plus denses ; le juge appartient par ailleurs à une \textbf{famille de modèle
...
rapport, le taux d'interprétabilité atteint \textbf{84,7\%}. L'\textbf{échelle
du modèle extracteur/juge}, identifiée dans des campagnes antérieures comme le
seul levier produisant un effet net et significatif, ne réplique cet effet à
aucune paire (1B/4B/12B, $p\geq0{,}29$) une fois les trois échelles mesurées
sous un protocole totalement homogène (sélection stratifiée, juge Qwen3.8-27B,
déduplication) --- un point de mesure (27B) reste en cours au moment de la
rédaction pour trancher si cet effet, jusqu'ici le plus robuste du stage,
survit à ce contrôle. Des tests complémentaires --- fidélité et plausibilité
...
sur les transitions énergétique, numérique et sociétale du secteur et conçoit
...
avec une initialisation \emph{iso} (gaussienne isotrope normalisée, le schéma
secondaire de Korznikov et al. --- leur schéma \emph{principal}, \emph{cov},
gaussien de covariance réelle et empiriquement plus difficile à battre, est en
cours de mesure séparément). Deux effets se combinent : d'abord, un décodeur
figé aléatoire laisse \textbf{98,9\% des 1024 directions de l'extension
mortes} (contre 5,7\% pour R0) --- l'encodeur n'a mécaniquement que 11
directions vivantes à proposer au juge stratifié, contre 150 attendues.
Ensuite, et c'est la mesure la plus nette des deux, la fraction de variance
expliquée gagnée par l'extension ($\Delta$FVE, indépendante du juge)
s'effondre à \textbf{$+0{,}0001$ contre $+0{,}1393$ pour R0} --- un facteur
${\sim}1400$ : la quasi-totalité de la capacité de l'extension ne sert à
rien, ni pour l'interprétabilité ni pour la reconstruction, sans direction de
décodage apprise. Sur les 11 features vivantes, le taux d'interprétabilité
reste plus bas que R0 (54,5\% contre 84,7\%, $z=2{,}54$, $p=0{,}011$) mais à
...
La campagne d'ablations qui précède (\S\ref{subsec:synthese-ablations})
sélectionne les features à juger par \textbf{magnitude d'activation moyenne},
avec gemma-3-12b-it comme juge --- le même modèle que celui dont les
activations sont extraites. Ces deux choix, tout aussi susceptibles
d'influencer le taux mesuré qu'un hyperparamètre du SAE, sont eux-mêmes
évalués comme deux ablations à part entière, contre deux alternatives : une
\textbf{sélection stratifiée} par bins de fréquence (échantillonnage équilibré
sur des strates log-espacées plutôt que sur les seules features les plus
actives), et un \textbf{juge de famille différente} (Qwen3.8-27B), pour
écarter un biais d'auto-préférence.

La sélection stratifiée s'avère être, de loin, le second effet le plus large
mesuré dans ce stage : sur le même SAE et le même corpus, le taux passe de
45,3\% (68/150, sélection par magnitude) à 89,3\% (134/150, sélection
stratifiée) --- écart hautement significatif ($z=-8{,}12$,
$p=4{,}5\times10^{-16}$). La sélection par magnitude favorise systématiquement
les features les plus denses, proches de directions génériques et donc plus
difficiles à interpréter individuellement : un échantillon non représentatif
du dictionnaire complet, qui sous-estime structurellement le taux réel.

Rejugeant les mêmes features stratifiées avec Qwen3.8-27B, le taux mesuré
monte à \textbf{94,0\% (141/150, déduplication des exemples par mail parent
incluse)} --- Qwen se montre systématiquement un peu plus généreux que
gemma-3-12b-it. Sur un échantillon sélectionné par magnitude en revanche, le
même changement de juge produit un écart massif
(45,3\%~$\to$~78,7\%, McNemar apparié $\chi^2=31{,}59$, $p=1{,}9\times10^{-8}$) :
l'effet du choix de juge est \textbf{concentré sur les features denses} que la
sélection par magnitude sur-échantillonne, plutôt qu'une propriété générale du
dictionnaire. Cette campagne établit ainsi la \textbf{méthodologie corrigée}
retenue pour le reste du rapport (sélection stratifiée, juge Qwen3.8-27B,
déduplication par mail parent) --- mais elle porte sur une configuration
(\texttt{results\_v10\_emails\_main} : $K_\text{extra}=32$, 500\,000 tokens,
layer 24) distincte de celle choisie comme référence pour la campagne
d'ablations à variable unique isolée (\S\ref{subsec:synthese-ablations},
$K_\text{extra}=5$, 25M tokens, layer 31) --- comparer directement 94,0\% aux
chiffres de cette dernière confondrait deux configurations qui diffèrent sur
trois axes à la fois, l'erreur exacte que \S\ref{subsec:model-scale}
identifie et corrige pour l'effet d'échelle.

\begin{resultbox}[Valeur de référence retenue]
Sous la méthodologie ci-dessus, appliquée à la configuration retenue comme
référence de la campagne d'ablations ($K_\text{extra}=5$, $D_\text{extra}=1024$,
25M tokens, layer 31, gemma-3-12b-it) : \textbf{84,7\% (127/150) est le taux
d'interprétabilité de référence de ce rapport.} Le 94,0\% ci-dessus
(\texttt{results\_v10\_emails\_main}) reste la mesure qui a \emph{validé} cette
méthodologie mais n'est plus cité comme référence, faute de partager la même
configuration que le reste de la campagne d'ablations. Les chiffres en
45,x\%/2x,x\% cités dans la campagne d'ablations d'hyperparamètres du SAE
...
sont désormais comparables terme à terme, seule l'échelle du modèle varie.
...
gemma-3-27b-it & 40 & Qwen3.8-27B & \emph{en cours au moment de la rédaction} \\
\bottomrule
\end{tabular}
\end{table}

\begin{resultbox}[Résultat central du stage --- à confirmer par le point 27B]
Sous protocole totalement homogène, la progression 1B$\to$4B$\to$12B
(80,0\%~$\to$~81,3\%~$\to$~84,7\%) \textbf{n'atteint la significativité à
aucune paire} (1B vs 12B : $z=-1{,}06$, $p=0{,}29$ ; 1B vs 4B : $z=-0{,}29$,
$p=0{,}77$ ; 4B vs 12B : $z=0{,}77$, $p=0{,}44$) ni en tendance
(Cochran-Armitage sur les trois points, $z=1{,}05$, $p=0{,}29$). C'est un
résultat que ce rapport n'anticipait pas : la version antérieure de cette
section, construite en mélangeant des points jugés par des modèles
différents, décrivait une progression nette jusqu'à 12B puis un plateau ---
cette lecture ne survit pas au retrait du biais de juge. \textbf{Le point 27B,
en cours, est décisif} : s'il rouvre un écart significatif avec 12B,
l'échelle du modèle reste le levier dominant identifié dans les campagnes
antérieures (§28, §82, §95), avec une ampleur nettement revue à la baisse ;
s'il confirme l'absence d'effet, alors, sous cette méthodologie, \textbf{aucun
des leviers testés dans ce stage} --- ni les hyperparamètres du SAE
(\S\ref{subsec:synthese-ablations}) ni l'échelle du modèle extracteur/juge ---
ne produit d'effet mesurable sur le taux d'interprétabilité à $n=150$. Les
deux issues sont possibles à ce stade de la rédaction ; ce paragraphe sera mis
à jour dès que le point 27B sera disponible, pas requalifié une seconde fois
par petites touches.
\end{resultbox}
...
A3 & core 65k (défaut : 16k) & \emph{en cours} & --- & \emph{en cours} \\
A4 & $\text{batch}_\text{extra}=16384$ (défaut : 1024) & \emph{en cours} & --- & \emph{en cours} \\
A6 & $\text{épochs}_\text{extra}=40$ (défaut : 10) & \emph{en cours} & --- & \emph{en cours} \\
\bottomrule
\end{tabular}
\end{table}
...
plus stable : $+0{,}1393$ à $+0{,}1483$ sur les cinq runs, un écart relatif
