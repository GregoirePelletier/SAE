...
corrigé --- à l'exception de deux leviers réels. \textbf{L'échelle du modèle
extracteur/juge} produit un effet dose-réponse net et hautement
significatif (12,0\% à 1B paramètres, 28,0\% à 4B, 45,3\% à 12B ; test de
tendance de Cochran-Armitage, $p \approx 1{,}6\times10^{-10}$) qui
\textbf{plafonne} ensuite : un palier à 27B, testé ultérieurement sous la
méthode corrigée ci-dessous, n'apporte plus de gain significatif par rapport
à 12B (83,3\% contre 82,0\%, $p=0{,}76$). \textbf{La méthode de sélection des
features à juger} s'est révélée être le second levier, plus important encore :
la sélection par magnitude d'activation (utilisée pour tous les chiffres
ci-dessus) favorise systématiquement les features les plus denses, un
échantillon non représentatif du dictionnaire --- remplacée par une sélection
stratifiée par bins de fréquence, le taux mesuré sur le même SAE passe de
45,3\% à 89,3\% ($z=-8{,}12$, $p=4{,}5\times10^{-16}$). Un troisième correctif,
portant sur le juge lui-même (gemma-3-12b-it jugeait jusqu'ici ses propres
features), donne 94,7\% en rejugeant les mêmes features stratifiées avec un
modèle de famille différente (Qwen3.8-27B), écart non significatif à cette
échelle ($p=0{,}057$) mais dans le sens attendu d'un biais d'auto-préférence.
\textbf{94,7\% (sélection stratifiée, juge Qwen) est la valeur de référence
retenue pour ce rapport} ; les chiffres en 45,x\%/2x,x\% cités dans les
sections d'ablation datent tous de la méthode de sélection par magnitude et du
juge auto-référent, et se lisent comme un plancher, pas comme le taux réel du
dictionnaire. Des tests complémentaires (fidélité et plausibilité de
l'explication document-level, sanity check contre un décodeur figé aléatoire,
robustesse du protocole de jugement, biais multilingue, fidélité du steering,
évaluation quantitative du retrieval --- fusion RRF et reranking LLM, qui
dominent uniformément TF-IDF et le retrieval sémantique seul) complètent la
validation du système, avec un audit rétroactif de la méthodologie statistique
employée (tests appariés, correction multi-tests).
...
corpus domain is fixed --- except for two real levers. \textbf{The scale of the
extractor/judge model} produces a clean, highly significant dose-response
effect (12.0\% at 1B parameters, 28.0\% at 4B, 45.3\% at 12B; Cochran-Armitage
trend test, $p \approx 1.6\times10^{-10}$) that then \textbf{plateaus}: a 27B
tier, tested later under the corrected method below, brings no further
significant gain over 12B (83.3\% vs 82.0\%, $p=0.76$). \textbf{The
feature-selection method used for judging} turned out to be the second, even
larger lever: selection by activation magnitude (used for every figure above)
systematically favors the densest features, an unrepresentative sample of the
dictionary --- replaced by frequency-stratified sampling, the rate measured on
the same SAE rises from 45.3\% to 89.3\% ($z=-8.12$, $p=4.5\times10^{-16}$). A
third correction, targeting the judge itself (gemma-3-12b-it had until then
judged its own features), gives 94.7\% when the same stratified features are
rejudged by a different model family (Qwen3.8-27B), a gap not significant at
this scale ($p=0.057$) but pointing in the direction expected from a
self-preference bias. \textbf{94.7\% (stratified selection, Qwen judge) is
this report's reference value}; the 45.x\%/2x.x\% figures cited in the
ablation sections all predate the selection-method and judge corrections and
should be read as a floor, not the dictionary's true rate. Complementary
...
quantitative retrieval evaluation --- RRF fusion plus LLM reranking, which
uniformly beat TF-IDF and semantic retrieval alone) complete the system's
validation, together with a retroactive audit of the statistical methodology
used (paired tests, multiple-comparison correction).
...
\emph{cf.}~\S\ref{subsec:robustesse}). La majorité des résultats de ce
rapport (sections d'ablation, \S\ref{sec:resultats}) utilisent le même modèle
comme extracteur et comme juge --- un choix dont le stage a établi
\emph{a posteriori} qu'il n'était pas neutre (\S\ref{subsec:correction-methode}) :
la valeur de référence retenue (94,7\%) découple les deux rôles, un juge de
famille différente (Qwen3.8-27B) jugeant les features extraites par
Gemma-3-12B-it.
...
effet mesuré dans tout ce stage. Un palier supplémentaire à 27B, testé
ultérieurement sous la sélection stratifiée (\S\ref{subsec:correction-methode}),
confirme un \textbf{plateau} : 82,0\% à 12B, 83,3\% à 27B ($p=0{,}76$, non
significatif) --- doubler encore la taille du modèle au-delà de 12B n'apporte
plus de gain mesurable à cette échelle.
\end{resultbox}
...
\subsection{Second et troisième correctifs : sélection des features et choix du juge}
\label{subsec:correction-methode}

Les résultats ci-dessus, comme la table~\ref{tab:synthese}, sélectionnent les
features à juger par \textbf{magnitude d'activation moyenne} --- un choix qui
s'est révélé biaisé \emph{a posteriori} : cette méthode favorise
systématiquement les features les plus denses (proches de directions
génériques), un échantillon non représentatif du dictionnaire complet.
Remplacée par une \textbf{sélection stratifiée} par bins de fréquence
(échantillonnage équilibré sur des strates de fréquence log-espacées plutôt
que les features les plus actives), le taux mesuré sur le même SAE et le même
corpus passe de \textbf{45,3\% (68/150) à 89,3\% (134/150)} --- écart
hautement significatif ($z=-8{,}12$, $p=4{,}5\times10^{-16}$), et de loin le
second effet le plus large de tout ce stage.

Un second doute a ensuite porté sur le \textbf{juge lui-même} :
gemma-3-12b-it jugeait jusqu'ici ses propres features, le même checkpoint que
celui dont les activations sont extraites --- un risque de biais
d'auto-préférence jamais isolé jusqu'ici, faute d'un juge de famille
différente et de capacité comparable disponible localement. Rejugeant les
mêmes 150 features stratifiées avec Qwen3.8-27B (famille différente), le taux
mesuré monte à \textbf{94,7\% (142/150)}, écart non significatif à cette
échelle (test de McNemar exact, $p=0{,}057$) mais dans le sens attendu si un
biais d'auto-préférence existait (gemma-3-12b-it plus sévère avec lui-même).
Sur un échantillon de features sélectionnées par \emph{magnitude} en revanche,
le même changement de juge produit un écart massif et hautement significatif
(45,3\% $\to$ 78,7\%, McNemar apparié $\chi^2=31{,}59$, $p=1{,}9\times10^{-8}$)
--- l'effet du choix de juge semble donc concentré sur les features denses que
la sélection par magnitude sur-échantillonne, pas une propriété générale du
dictionnaire.

\begin{resultbox}[Valeur de référence retenue]
\textbf{94,7\% (sélection stratifiée, juge Qwen3.8-27B) est le taux
d'interprétabilité de référence de ce rapport.} Les chiffres en
45,x\%/2x,x\% cités dans les autres sections de ce chapitre datent de la
méthode de sélection par magnitude et du juge auto-référent : ils restent des
comparaisons \emph{internes} valides (les deux bras d'une même ablation,
mesurés sous la même méthode) mais leurs valeurs absolues se lisent comme un
...
Sur l'\textbf{interprétabilité} (pas seulement la reconstruction), le seul
chiffre disponible avant ce rapport datait d'une méthode de sélection et de
juge depuis corrigées côté Pipeline~1 mais jamais portées sur Pipeline~2.
Sous protocole comparable (sélection stratifiée, juge Qwen3.8-27B), le taux
mesuré est de \textbf{58,7\% (88/150)} --- sensiblement plus bas que la
référence Pipeline~1 (94,7\%), alors que la reconstruction Pipeline~2 est
elle-même bonne et non dégradée. Pipeline~2 fonctionne de bout en bout et
reconstruit bien, mais son niveau d'interprétabilité individuelle des
features reste, à ce stade, inférieur à celui de Pipeline~1.
...
\begin{itemize}
  \item L'architecture Matryoshka pleinement hiérarchique (dictionnaires
    ...
    suite.
  \item Séparer les rôles d'extracteur et de juge (utiliser un modèle léger
    pour l'extraction mais un juge plus capable) a depuis été fait
    (\S\ref{subsec:correction-methode}) : rejuger les mêmes features avec un
    juge de famille différente (Qwen3.8-27B au lieu de gemma-3-12b-it, le
    modèle extracteur) isole un effet de \textbf{choix du juge} massif sous
    sélection par magnitude (45,3\%~$\to$~78,7\%) mais non significatif sous
    sélection stratifiée (89,3\%~$\to$~94,7\%, $p=0{,}057$) --- l'effet
    d'échelle mesuré \S\ref{subsec:model-scale} reste donc, pour l'essentiel,
    un effet de capacité de raisonnement du juge plutôt qu'un biais
    d'auto-préférence, mais concentré sur les features que la sélection par
    magnitude sur-échantillonne. Reste ouvert : isoler explicitement la
    capacité d'\emph{extraction} de celle du \emph{jugement} demanderait un
    plan croisé (extracteur léger $\times$ juge capable, et l'inverse), non
    fait ici.
