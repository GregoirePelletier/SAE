...
# ══════════════════════════════════════════════════════════════════════════════
# FROZEN-CORE DOMAIN ADAPTATION (slide 8)
# ══════════════════════════════════════════════════════════════════════════════

class FrozenCoreResidualSAE(nn.Module):
    """
    Cœur gelé + extension résiduelle d_extra features.

    Architecture (FIXED v2) :
      - core_sae : SAE préentraîné GemmaScope, poids totalement figés
      - Extension : encode le résiduel e = x - core_out, pas x directement
      - Perte : MSE(extra_out, e)  →  n'optimise que l'information résiduelle
      - PAS de b_dec_extra (préserve le comportement nul sur anglais)

    Propriété algébrique clé :
      x_hat = core_out + extra_out = core_out + z_extra @ W_dec_extra
      Sur texte EN : z_extra ≈ 0  ⟹  x_hat ≈ core_out  (oubli catastrophique nul)
    """
    def __init__(self, core_sae: SAE, d_extra: int = D_EXTRA, k_extra: int = K_EXTRA):
        super().__init__()
        ...
        self.W_enc_extra = nn.Parameter(W_dec.T.clone())  # [d_in, d_extra]
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        """Retourne les activations concaténées [core | extra] — [B, d_core + d_extra]."""
        with torch.no_grad():
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        extra_out  = extra_acts @ self.W_dec_extra   # no b_dec_extra

        mse_loss = F.mse_loss(extra_out, residual)
        var_x = (x - x.mean(dim=0)).pow(2).mean()
        nmse  = mse_loss / (var_x + 1e-8)  # Normalisation sur la variance globale de x
...
class ExtendedSAE(FrozenCoreResidualSAE):
    """
    Variante avec initialisation des directions décoders par PCA du résiduel
    (SAEBoost Table 7 / Koriagin et al.).
    Hérite de FrozenCoreResidualSAE — même forward, seule l'init change.
    """
    def __init__(self, core_sae: SAE, d_extra: int = D_EXTRA, k_extra: int = K_EXTRA,
                 domain_residuals: torch.Tensor = None):
        super().__init__(core_sae, d_extra, k_extra)
        if domain_residuals is not None:
            self._init_from_residual_pca(domain_residuals)

    def _init_from_residual_pca(self, residuals: torch.Tensor):
        """W_dec_extra ← top-k directions PCA du résiduel e = x - core_out."""
...
class PhraseLevelSAE(nn.Module):
    """BatchTopK SAE entraîné depuis zéro sur embeddings F2LLM phrase-level."""
...
def extract_f2llm_embeddings(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        emb_model, token=HF_TOKEN, trust_remote_code=True, local_files_only=True
    )
...
# ══════════════════════════════════════════════════════════════════════════════
# MAX-POOL SAE ACTIVATIONS PHRASE → DOC (Pipeline 2)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# LOAD / TRAIN PHRASE SAE (Pipeline 2)
# ══════════════════════════════════════════════════════════════════════════════
...
def load_or_train_sae(
    ...
    if os.path.exists(save_path):
        ...
        cfg = ckpt.get("config", {})
        if cfg.get("d_in") and cfg["d_in"] != d_in:
            raise ValueError(f"Désalignement d_in checkpoint ({cfg['d_in']}) ≠ session ({d_in})")
...
# ══════════════════════════════════════════════════════════════════════════════
# MÉTRIQUES SAE (Pipeline 2)
# ══════════════════════════════════════════════════════════════════════════════

def compute_sae_metrics(sae: PhraseLevelSAE, embeddings: torch.Tensor,
