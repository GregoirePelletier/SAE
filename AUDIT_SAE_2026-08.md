...
| 3 | Fragments shardés + écriture asynchrone | `fragment_store.py`, `saev5.py:920` | 2–2,5× sur le wall-clock d'extraction | 1 j | **fait, testé (CPU)**, GPU en attente. `ShardedFragmentWriter` : SHARD_SIZE=1000 documents par fichier (`shards/shard_{k:05d}.pt`), écriture toujours en arrière-plan (encapsule un `AsyncFragmentWriter`). Utilisé **uniquement côté extraction** — le ré-encodage continue d'écrire un fichier par document via `save_fragment()` (inchangé, déjà testé), qui **prime** sur le shard à la lecture (`load_fragment`) : un document ré-encodé "sort" proprement de son shard. Reprise en cas de coupure en cours de shard : `add()` précharge le contenu déjà sur disque au premier contact avec une shard (sinon un flush ultérieur écraserait les documents d'avant la coupure). Bug de péremption de cache trouvé et corrigé en écrivant les tests : un shard lu (mis en cache pour accélérer les lectures séquentielles) puis modifié par le writer pouvait être relu périmé — `ShardedFragmentWriter.flush()` invalide désormais le cache après confirmation d'écriture sur disque. Nettoyage : les shards sont supprimés (`shutil.rmtree`) une fois le ré-encodage **naturellement** complet (jamais sur early-exit) puisque `re_encode_targets` couvre 100 % des documents fragmentés (filler jamais fragmenté, G7) — sans ce nettoyage, chaque document fragmenté existerait deux fois sur disque (shard + fichier individuel), doublant l'espace pour rien. Testé (CPU, `tests/test_fragment_sharding.py`) : round-trip multi-shards, reprise en shard partielle, priorité fichier-individuel-sur-shard, régression du bug de cache périmé. Pas encore vérifié sur GPU réel (changement de format sur disque, tenu à l'écart de tout run en cours jusqu'ici) |
...
| 7 | `BATCH_SIZE` 1024 → 16 384 + `.item()` hors boucle | `sae_shared.py:178` | 5–15× sur l'entraînement extra | 3 h (+ ablation BatchTopK) | **partiellement fait** : `.item()`/`float()` par step → accumulation GPU + 1 sync/époque, fait et testé (CPU, valeurs identiques bit-à-bit à l'ancien code). `BATCH_SIZE` paramétré (`BATCH_SIZE_EXTRA`, `src/config.py`, défaut 1024 inchangé) plutôt que codé en dur, pour permettre l'ablation sans changer le défaut à l'aveugle. Ablation **terminée** (job 44620) : `BATCH_SIZE_EXTRA=16384` donne dead_pct 53,3 % (contre ~57,8 % aux `BATCH_SIZE_EXTRA=1024` de référence — moins de features mortes) MAIS rho_sae 0,781 (contre ~0,82-0,83 — fidélité de reconstruction dégradée). **Pas un gain net** : compromis réel dans le sens attendu (le budget top-k partagé sur un batch plus large change la distribution de sparsité vue à l'entraînement), pas juste un facteur de vitesse. N'adopter `BATCH_SIZE_EXTRA=16384` par défaut qu'après réplication multi-graines (n=1 ici) — ne PAS déployer tel quel malgré le gain de vitesse. |
| 8 | Batcher le ré-encodage (64–256 docs) | `saev5.py:1069` | 5–10× sur cette passe | 4 h | **fait, testé (CPU)**, GPU en attente. `REENCODE_BATCH_SIZE` (défaut 128, `src/config.py`) : les fragments d'un lot sont concaténés (CPU puis un seul transfert device) pour UN SEUL appel `_encode_extra_acts`, au lieu d'un par document — équivalence exacte car `ext_sae.eval()` place `BatchTopKEncoder` en seuil global élément-par-élément (pas le budget partagé par batch de l'entraînement). Le reste (merge CSR, écriture fragment, max-pool, granularité de checkpoint) reste par document, inchangé. Testé (CPU, `tests/test_reencode_batching_equivalence.py`) : équivalence bit-exacte batché vs séquentiel, y compris à des tailles de lot variables (position dans le lot sans effet). **Vérifié sur GPU** (job 44671, réplique de 44572/v3) : 44 253 documents, 63 min 46 (11,56 doc/s, non batché, 44572) → 54 min 46 (13,47 doc/s, batché) sur cette phase, **−14,1 % temps mur**. rho_sae/dead_pct identiques bit-à-bit à 44659 (autre run avec le même code sauf ce correctif) — confirme sur GPU réel l'équivalence déjà prouvée sur CPU, aucune différence de fidélité attribuable au batching. |
...
| 11 | TF32/SDPA/`inference_mode`/`expandable_segments` | global | 3–8 % cumulés | 1 h | partiellement fait (`set_float32_matmul_precision` désormais posé dans `saev5.py` en plus de `phrase_sae.py`/`latent_terms.py` ; `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` ajouté à `pipeline_runs/`, vérifié sur GPU job 44659, terminé sans erreur, rho_sae/dead_pct dans le bruit habituel entre runs identiques) ; SDPA jugé probablement déjà no-op (transformers 5.12.1, auto-détecté), `inference_mode` laissé de côté (risque de régression réel, pas vérifié) |
