...
| 7 | `BATCH_SIZE` 1024 → 16 384 + `.item()` hors boucle | `sae_shared.py:178` | 5–15× sur l'entraînement extra | 3 h (+ ablation BatchTopK) | **partiellement fait** : `.item()`/`float()` par step → accumulation GPU + 1 sync/époque, fait et testé (CPU, valeurs identiques bit-à-bit à l'ancien code). `BATCH_SIZE` non touché — change le régime de sparsité BatchTopK (budget partagé par batch), reste une ablation à faire, pas un correctif sûr par défaut |
